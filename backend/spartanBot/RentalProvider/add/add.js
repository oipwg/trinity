...
module.exports = function(options) {
    ...
    async function add(args) {
        ...
        if (rentalProviders.length === 2)
            return console.log('Maximum number of providers reached.');

        let select_rental_providers = await Prompt_RentalProviders(
            self,
            vorpal,
            spartan
        );
        ...
        if (rental_provider_type === MiningRigRentals) {
            if (checkProviders(MiningRigRentals))
                return console.log(`MiningRigRentals account already exists.
                                Current Limit: 1.`);
            ...
            if (checkProviders(NiceHash))
                return self.log(
                    vorpal.chalk.red(
                        `NiceHash account already exists. ${vorpal.chalk.cyan(
                            'Current Limit: 1.'
                        )} `
                    )
                );
        ...
        try {
            ...
            if (setup_success.success) {
                ...
                this.log(
                    vorpal.chalk.white(
                        'Successfully added a new Rental Provider.'
                    )
                );
                ...
                if (setup_success.message === 'settings.api_key is required!') {
                    ...
                    setup_success.message === 'settings.api_secret is required!'
                ) {
                    this.log(vorpal.chalk.red('You must input an API Secret!'));
                } else if (
                    setup_success.message === 'Provider Authorization Failed'
                ) {
                    this.log(
                        vorpal.chalk.red(
                            'Unable to login to Account using API Key & API Secret, please check your keys and try again'
                        )
                    );
                }
            }
            ...
            this.log(
                vorpal.chalk.red('Error! Unable to add Rental Provider!\n' + e)
            );
        }
    }
};
