...
exports.Prompt_RentalProviders = async (self, vorpal, spartan) => {
    const exit = vorpal.chalk.red(`exit`);
    return await self.prompt({
        type: 'list',
        name: 'rental_provider',
        message: vorpal.chalk.yellow(
            'Which Rental Provider would you like to add?'
        ),
        choices: [...spartan.getSupportedRentalProviders(), exit],
    });
};

exports.Prompt_MRRAPIKeys = async (self, vorpal) => {
    return await self.prompt([
        {
            type: 'input',
            name: 'api_key',
...
exports.Prompt_NiceHashAPIKeys = async (self, vorpal) => {
    return await self.prompt([
        {
            type: 'input',
            name: 'api_key',
...
exports.Prompt_OptionalName = async (self, vorpal) => {
    return await self.prompt({
        type: 'input',
        name: 'name',
        message: vorpal.chalk.yellow(
            'Add an optional name to your rental provider: '
        ),
        default: 'undefined',
    });
};

exports.Prompt_AddOrCreatePool = async (self, vorpal, provider) => {
    const exit = vorpal.chalk.red(`exit`);
    let message =
        provider.getInternalType() === 'MiningRigRentals'
            ? vorpal.chalk.yellow(
                  'Select a pool profile to use or create a new one'
              )
            : vorpal.chalk.yellow(
                  'Add an existing pool or a create a new pool'
              );
    let choices =
        provider.getInternalType() === 'MiningRigRentals'
            ? ['select', 'create', exit]
...
exports.Prompt_AddPool = async (self, vorpal, poolArray) => {
    const exit = vorpal.chalk.red(`exit`);
    return await self.prompt({
        type: 'list',
        name: 'option',
...
exports.Prompt_CreateMRRPool = async (self, vorpal, spartan) => {
    let poolOptions = {};
    ...
    let user = await self.prompt({
        ...
        name: 'user',
        message: vorpal.chalk.yellow(
            'Input a wallet address to receive funds at: '
        ),
        description: 'Your workname',
        default: spartan.wallet.wallet.coins['flo']
            .getMainAddress()
            .getPublicAddress(),
    });
    ...
    let pass = await self.prompt({
        ...
        name: 'pass',
        message: vorpal.chalk.yellow(
            `${vorpal.chalk.red('Optional')} add a password: `
        ),
        default: 'x',
    });
    ...
    let notes = await self.prompt({
        ...
        name: 'notes',
        message: vorpal.chalk.yellow(
            `${vorpal.chalk.red('Optional')} add any additional notes: `
        ),
...
exports.Prompt_CreatePoolProfile = async (self, vorpal, spartan) => {
    let poolOptions = {};
    ...
    let algo = await self.prompt({
        ...
        name: 'algo',
        message: vorpal.chalk.yellow(
            'Input an algorithm to mine with (scrypt, x11, sha256, etc...): '
        ),
        default: 'scrypt',
    });
    ...
    let priority = await self.prompt({
        ...
        name: 'priority',
        message: vorpal.chalk.yellow(
            'What priority would you like this pool to be at?: '
        ),
        choices: ['0', '1', '2', '3', '4'],
    });
...
exports.Prompt_NiceHashCreatePool = async (self, vorpal, spartan) => {
    let NiceHashPool = {};
    let type = await self.prompt({
        type: 'input',
        name: 'type',
